...
>
	<div class="mx-auto flex items-center justify-between sm:px-6 md:max-w-2xl lg:max-w-4xl lg:px-12">
		<span class="lg:hidden">
			<Button noOutline size="sm" variant="primary" href="#pricing">start</Button>
		</span>
			...
			<div class="max-w-[165px]">
				{#if is_scrolled}
					<div transition:slide>
						<Button noOutline noShadow size="lg" variant="primary">Start Browser</Button>
					</div>
				{/if}
				{#if !is_scrolled}
					<div transition:slide>
						<Button noOutline noShadow size="lg" variant="ghost">Start Browser</Button>
					</div>
				{/if}
			</div>
		</div>
